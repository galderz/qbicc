...
// TODO rename to EscapeOptimizeBasicBlockBuilder if it works
public class EscapeCopyVisitor implements NodeVisitor.Delegating<Node.Copier, Value, Node, BasicBlock, ValueHandle> {
    private final CompilationContext ctxt;
    private final NodeVisitor<Node.Copier, Value, Node, BasicBlock, ValueHandle> delegate;

    public EscapeCopyVisitor(final CompilationContext ctxt, final NodeVisitor<Node.Copier, Value, Node, BasicBlock, ValueHandle> delegate) {
        this.ctxt = ctxt;
...
//        // Get a copy of the value in the new graph
//        final Value copy = NodeVisitor.Delegating.super.visit(param, original);
//        System.out.println("visit(Node.Copier, New) : original: " + original + ", copy: " + copy);
//
//        // If the allocation is still present and it has not been changed,
//        // see if the allocation can be optimized.
//        if (copy instanceof New) {
//            // Update the escape analysis data with the new copy.
//            EscapeAnalysis.get(ctxt).copy(original, copy);
//
//            // TODO temporarily limit to example for easier testing
//            if (EscapeAnalysis.get(ctxt).notEscapingThread(copy) && copy.getType().toString().contains("example")) {
//                return stackAllocate(((New) copy).getClassObjectType(), param.getBlockBuilder());
//            }
//        }
